...
class GetIndicatorValues implements ShouldQueue
{
    ...
    public function handle()
    {
        switch ($this->indicator) {
            ...
            case 'TX_CURR':
                ...
                break;
            case 'TX_RTT':
                // $this->getTxRtt($this->period);
                break;
            case 'TX_ML':
                // $this->getTxMl($this->period);
                break;
            case 'TX_PVLS':
    ...
    // public function getTxRtt($period)
    // {
    //     //
    // }

    // public function getTxMl($period)
    // {
    //     //
    // }
    ...
    public function getRetentionOnArt12Months($period)
    {
        ...
        DB::connection('sqlsrv')->table('FACT_ART_Retention731')
            ->selectRaw('MFLCode as facility_code, MAX(FacilityName) as facility_name, SUM(Active12M) as value')
    ...
    public function getRetentionOnArtVl100012Months($period)
    {
        config(['database.connections.sqlsrv.database' => 'PortalDev']);
        DB::connection('sqlsrv')->table('FACT_ART_Retention731')
            ->selectRaw('MFLCode as facility_code, MAX(FacilityName) as facility_name, SUM(m12_Last12MVLSup) as value')
            ->whereNotNull('MFLCode')
            ->where('StartYear', $period->format('Y'))
            ->where('StartMonth', $period->format('n'))
            ->groupBy('MFLCode')
